...
data = {'alpha-2': ["AD", "AE", "AF", "AG", "AI", "AL", "AM", "AN", "AO", "AQ", "AR", "AS", "AT", "AU", "AW",
        ...
        'iso_alpha': ["AND", "ARE", "AFG", "ATG", "AIA", "ALB", "ARM", "ANT", "AGO", "ATA", "ARG", "ASM", "AUT", "AUS",
                      "ABW",
...
body_app = dbc.Container([
    ...
    dbc.Row(html.Marquee("USA, India and Brazil are top 3 countries in terms of confirmed cases"),
            style={'color': 'green'}),
    ...
    dbc.Row([
        ...
        dbc.Col(dbc.Card(data_for_cases("Recovered", f'{recovered:,}', f'{newrecovered:,}'), color='success',
                         style={'text-align': 'center'}, inverse=True), xs=12, sm=12, md=4, lg=4, xl=4,
                style={'padding': '12px 12px 12px 12px'}),
        dbc.Col(dbc.Card(data_for_cases("Deaths", f'{deaths:,}', f'{newdeaths:,}'), color='danger',
                         style={'text-align': 'center'}, inverse=True), xs=12, sm=12, md=4, lg=4, xl=4,
                style={'padding': '12px 12px 12px 12px'})
    ...
    dbc.Row([dbc.Col(dcc.Graph(id='world-graph', figure=world_map(df_world_f)), style={'height': '450px'}, xs=12, sm=12,
                     md=6, lg=6, xl=6),

             dbc.Col([html.Div(id='dropdown-div', children=
             [dcc.Dropdown(id='country-dropdown',
                           options=[{'label': i, 'value': i} for i in
                                    np.append(['All'], df_countries['Country'].unique())],
                           value='All',
...
navbar = dbc.Navbar(id='navbar', children=[
    ...
    html.A(
        dbc.Row([
            ...
            dbc.Col(
                dbc.NavbarBrand("Covid-19 Live Tracker",
...
def table_country(country):
    ...
    return dash_table.DataTable(
        ...
        columns=[{'id': c, 'name': c} for c in
                 df_final[['Country', 'TotalConfirmed', 'TotalRecovered', 'TotalDeaths']].columns],
